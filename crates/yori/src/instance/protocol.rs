...
use crate::{
    comparison::{ComparisonPaths, MergePaths},
    ...
    comparisons
        ...
        .map(|comparison| match comparison {
            ComparisonPaths::Diff { baseline, local } => Ok(WireComparison::Diff {
                baseline: encode_path(baseline, total_path_bytes, "file path")?,
                local: encode_path(local, total_path_bytes, "file path")?,
            }),
            ComparisonPaths::Merge(paths) => Ok(WireComparison::Merge {
                base: encode_path(&paths.base, total_path_bytes, "file path")?,
                local: encode_path(&paths.local, total_path_bytes, "file path")?,
                incoming: encode_path(&paths.incoming, total_path_bytes, "file path")?,
                result: encode_path(&paths.result, total_path_bytes, "file path")?,
            }),
        })
        .collect()
}
...
fn decode_comparisons(
    comparisons: Vec<WireComparison>,
    total_path_bytes: &mut usize,
) -> Result<Vec<ComparisonPaths>, String> {
    if comparisons.len() > MAX_COMPARISONS {
        return Err("too many comparisons in one request".into());
    }
    ...
    comparisons
        ...
        .map(|comparison| match comparison {
            WireComparison::Diff { baseline, local } => Ok(ComparisonPaths::Diff {
                baseline: decode_path(&baseline, total_path_bytes, "file path")?,
